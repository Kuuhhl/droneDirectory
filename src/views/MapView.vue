...
const updateBounds = () => {
  ...
  if (bounds) {
    ...
    console.log("Bounds updated: ", mapBounds.value)
  }
};
...
watch(() => googleMapRef.value?.ready, (ready) => {
  ...
  googleMapRef.value.map.addListener('zoom_changed', () => {
    zoomValue.value = googleMapRef.value.map.getZoom();
  });
})

const markerOptions = airport => ({
  title: `${airport.name} - No Fly Zone`,
  label: `${airport.name} - No Fly Zone`,
  ...
  <GoogleMap api-key="AIzaSyD1uFtiEOeXq5pzdiKT3QHzSFpe6L2v1lo" style="width: 100%; height: 80vh" :center="center"
    ...
    <template v-if="zoomValue >= 12">
      <Marker v-for="       airport       in       visibleAirports       " :key="airport.id"
        :options="markerOptions(airport)" />
    </template>
    <template v-if="zoomValue >= 8">
      <Circle v-for="       airport       in       visibleAirports       " :key="airport.id"
        :options="circleOptions(airport)" />
    </template <Marker v-if="locationFound" :options="{ position: center, ...currentLocationMarkerOptions }" />
  </GoogleMap>
</template>
